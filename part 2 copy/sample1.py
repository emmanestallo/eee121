...
def series_generate_linked_list(resistor_network, to_diverge, to_converge):
    series_linked_list = {}
    processed_resistors = set()
    iterations = 0  # Counter for iterations

    for res1 in resistor_network:
        if res1 in processed_resistors:
            continue

        for res2 in resistor_network:
            iterations += 1

            if res1 != res2 and res2 not in processed_resistors:
                start1, end1 = resistor_network[res1]
                start2, end2 = resistor_network[res2]

                if start2 == end1 and (len(to_converge[start2]) <= 1) and (len(to_diverge[start2]) <= 1):
                    series_linked_list[res1] = res2
                    processed_resistors.add(res2)

                elif start1 == end2 and (len(to_converge[start1]) <= 1) and (len(to_diverge[start1]) <= 1):
                    series_linked_list[res2] = res1
                    processed_resistors.add(res1)
