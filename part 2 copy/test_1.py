...
def series_identify_connection(r1, r2, resistor_network, to_diverge, to_converge):
    series_linked_list = series_generate_linked_list(resistor_network, to_diverge, to_converge)

    r1_is_in_linked_list = r1 in series_linked_list.keys()
    r2_is_in_linked_list = r2 in series_linked_list.keys()
    ...
    for r in series_linked_list.keys():
...
n = 0
q = 0
resistor_network = defaultdict(list)
...
def identify_connection_type(r1, r2):
    if series_identify_connection(r1, r2, resistor_network, to_diverge, to_converge):
        return "SERIES"
...
for count, line in enumerate(lines):
    if count == 0:
        ...
        res1, res2 = line.replace("\n", "").split(" ")
        print(identify_connection_type(res1, res2))
    else:
        resName, start, end, value = line.replace("\n", "").split(" ")
        value = float(value)
